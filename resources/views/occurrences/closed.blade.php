...
@section('content-header')
    <div class="content-header-left col-10 mb-2 mt-1">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h5 class="content-header-title float-left pr-1 mb-0">Serviços - Realizados
                    <small>({{$occurrences_all}})</small></h5>
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb p-0 mb-0">
                        <li class="breadcrumb-item"><i class="bx bx-home-alt"></i> Home</li>
                        <li class="breadcrumb-item">Serviços</li>
                        <li class="breadcrumb-item active">Realizados -
                            <small>Lista todas as Ocorrências realizadas</small></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="box-title">Serviços - Realizados</h3>
                </div>
                                ...
                                <table class="table table-condensed table-striped table-sm table-hover">
                                    ...
                                    @foreach($occurrences as $occurrence)
                                        <tr>
                                            ...
                                            <td>
                                                @if($occurrence->approved == 1)
                                                    <i class="bx bx-check-circle-o iconComunicated" data-widget="Já aprovada" data-toggle="tooltip" title="Já aprovada pelo setor administrativo "></i>
                                                @else
                                                    <i class="bx bx-circle-o iconNotComunicated" data-widget="Ainda não aprovada" data-toggle="tooltip" title="Ainda não foi aprovada pelo setor administrativo "></i>
                                                @endif
                                            </td>
                                            @if(!empty($occurrence->operator_id))
                                                <td>{!! (optional($occurrence->operator)->status != 1)? "<strike>" : "" !!}{{(empty($occurrence->operator_id)? "" : optional($occurrence->operator)->name)}}{!! (optional($occurrence->operator)->status != 1)? "</strike>" : "" !!}</td>
                                            @else
                                                <td>-</td>
                                            @endif
